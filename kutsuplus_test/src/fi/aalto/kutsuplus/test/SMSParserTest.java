package fi.aalto.kutsuplus.test;

import fi.aalto.kutsuplus.sms.SMSParser;
import junit.framework.TestCase;

public class SMSParserTest extends TestCase {
	protected String testMsgOK;
	protected String testMsgError;
	protected String testMsgInfo;

	public SMSParserTest() {
		super();
	}
	
	protected void setUp() {
		testMsgOK = "Pickup 11:08, stop 1901\n" +
							"Vehicle K11\n" +
							"Code 4U\n" +
							"1 pax\n" +
							"Drop-off 11:41+/-10min, stop E1129\n" +
							"6,74e\n" +
							"Order Id 2013-11-27-58488\n" +
							"https://kutsuplus.fi/t/4Upnw";
		testMsgError = "Pickup 11:08, stop 1901\n" +
							"Vehicle K11\n";
		testMsgInfo = "There are no vehicles available. https://www.kutsuplus.fi";
	}
	
	public void testParseOK() {
		SMSParser parser = new SMSParser();
		String[] result = parser.parse(testMsgOK);
		assertEquals(result[0], "11:08");
		assertEquals(result[1], "1901");
		assertEquals(result[2], "K11");
		assertEquals(result[3], "4U");
		assertEquals(result[4], "1");
		assertEquals(result[5], "11:41+/-10min");
		assertEquals(result[6], "E1129");
		assertEquals(result[7], "6,74");
		assertEquals(result[8], "2013-11-27-58488");
		assertEquals(result[9], "https://kutsuplus.fi/t/4Upnw");
	}

	public void testParseError() {
		SMSParser parser = new SMSParser();
		String[] result = parser.parse(testMsgError);
		assertEquals(result[0], testMsgError);
	}
	
	public void testParseInfo() {
		SMSParser parser = new SMSParser();
		String[] result = parser.parse(testMsgInfo);
		assertEquals(result[0], testMsgInfo);
	}
}
